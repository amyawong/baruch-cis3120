...
# 5. Set the keywords to be entered into the search bar
keywords = ["wireless headphones", "usb drive", "usb-c charger"]

"""
# 5.1. Following lines of code are for only one item:
keyword = 'robotic vacuum cleaner' 

# 5.2. Select the search box by using its ID
search = driver.find_element(By.ID, 'twotabsearchtextbox')

# 5.3. Use send keys
search.send_keys(keyword)

# 5.4. Select the search button by using its id
search_button = driver.find_element(By.ID, 'nav-search-submit-button')

# 5.5. Click the search button by using .click()
search_button.click()

# 5.6. Wait for 5 seconds for the page to load
driver.implicitly_wait(5)

# 5.7. Clear the search field using .clear()
search = driver.find_element(By.ID, 'twotabsearchtextbox').clear()
"""
...
for keyword in keywords:
  ...
  for item in scraped_items:
    ...
    link = item.find_element(By.XPATH, './/a[@class="a-link-normal s-underline-text s-underline-link-text s-link-style a-text-normal"]').get_attribute("href")
      ...
      # 22. Append scraped information to respective lists and convert the relevant columns to floast
    asin_list.append(asin)
    product_name_list.append(product_name.text)
    product_price_list.append(float(product_price))
    product_ratings_list.append(float(product_ratings.split('out')[0]))
...
# 29. Print description of dataframe
print(df.describe())

# 30. Export the data to a .csv file
